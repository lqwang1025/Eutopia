...
static std::string get_current_cate() {
    time_t nowtime;  
    nowtime = time(NULL);
    char tmp[64];   
    strftime(tmp,sizeof(tmp),"%Y年%m月%d号 %H:%M:%S",localtime(&nowtime));   
    return tmp;
}
...
static void value_info_init(onnx::ValueInfoProto* value_info, const Node* node) {
    value_info->set_name(node->get_name());
    onnx::TypeProto* tp = new onnx::TypeProto;
    onnx::TypeProto_Tensor* tensor = new onnx::TypeProto_Tensor;
    tensor->set_elem_type(static_cast<onnx::TensorProto_DataType>(node->get_output_tensor()->get_data_type()));
    for (auto it : node->get_output_shape()) {
        tensor->mutable_shape()->add_dim()->set_dim_value(it);
    }
    tp->set_allocated_tensor_type(tensor);
    value_info->set_allocated_type(tp);
}

void Graph::to_proto() const {
    onnx::ModelProto* model = new onnx::ModelProto;
    model->set_ir_version(1);
    std::string date = get_current_cate();
    onnx::StringStringEntryProto* time = model->add_metadata_props();
    time->set_key("Timestamp");
    time->set_value(date);
    onnx::StringStringEntryProto* desc = model->add_metadata_props();
    desc->set_key("Author");
    desc->set_value("Generated by @Daniel Wang.");
    ...
    for (int i = 0; i < (int)seq_nodes_.size(); ++i) {
        ...
        if (cur_node->is_last_node()) {
            onnx::ValueInfoProto* value_info = graph->add_output();
            value_info_init(value_info, cur_node);
        } else {
            onnx::ValueInfoProto* value_info = graph->add_value_info();
            value_info_init(value_info, cur_node);
        }
    }
...
// bool read_proto_from_txt(const char *filepath, google::protobuf::Message *message) {
